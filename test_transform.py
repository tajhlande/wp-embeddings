...
def test_pca_batch_sufficient():
    ...
    for i in range(150):
        ...
        conn.execute(
            "INSERT INTO page_log (page_id, chunk_name) VALUES (?, ?)", (i, chunk_name)
        )
        conn.execute(
            "INSERT INTO page_vector (page_id, embedding_vector) VALUES (?, ?)",
            (i, vector.tobytes()),
        )
...
def test_kmeans_basic():
    ...
    for i in range(n_vectors):
        ...
        conn.execute(
            "INSERT INTO page_vector (page_id, embedding_vector, reduced_vector) VALUES (?, ?, ?)",
            (
                i,
                np.random.randn(2048).astype(np.float32).tobytes(),
                reduced_vector.tobytes(),
            ),
        )
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT COUNT(*) as count FROM page_vector WHERE cluster_id IS NOT NULL"
        )
        result = cursor.fetchone()
        ...
        cursor = conn.execute(
            "SELECT COUNT(*) as count FROM cluster_info WHERE namespace = ?",
            (namespace,),
        )
        result = cursor.fetchone()
        ...
        print(
            f'Cluster assignments: {[(row["cluster_id"], row["count"]) for row in cluster_assignments]}'
        )
        cluster_id_set = set()
        for cluster in cluster_assignments:
            assert cluster[1] > 0, f"Empty cluster found: {cluster[0]}"
            assert (
                cluster[0] not in cluster_id_set
            ), f"Duplicate cluster ID found: {cluster[0]}"
...
def test_kmeans_centroids():
    ...
    for cluster_id in range(n_clusters):
        ...
        center = (
            np.random.randn(target_dim).astype(np.float32) * 2
        ...
        for i in range(n_vectors // n_clusters):
            ...
            conn.execute(
                "INSERT INTO page_log (page_id, chunk_name) VALUES (?, ?)",
                (page_id, chunk_name),
            )
            conn.execute(
                "INSERT INTO page_vector (page_id, embedding_vector, reduced_vector) VALUES (?, ?, ?)",
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT COUNT(*) as count FROM cluster_info WHERE centroid_vector IS NOT NULL AND namespace = ?",
            (namespace,),
        )
        result = cursor.fetchone()
        ...
        assert (
            result["count"] == n_clusters
        ), f"Expected {n_clusters} clusters with centroids, got {result['count']}"

        # Fetch and verify each centroid
        cursor = conn.execute(
            "SELECT cluster_id, centroid_vector FROM cluster_info WHERE namespace = ? ORDER BY cluster_id",
            (namespace,),
        )
        ...
        for row in stored_centroids:
            ...
            assert (
                len(stored_centroid) == target_dim
            ), f"Centroid {cluster_id} has wrong dimensionality: {len(stored_centroid)}, expected {target_dim}"

            # Verify centroid is not all zeros (basic sanity check)
            assert not np.allclose(
                stored_centroid, 0
            ), f"Centroid {cluster_id} is all zeros"
        ...
        for assignment in cluster_assignments:
            ...
            cursor = conn.execute(
                "SELECT 1 FROM cluster_info WHERE cluster_id = ? AND namespace = ? AND centroid_vector IS NOT NULL",
